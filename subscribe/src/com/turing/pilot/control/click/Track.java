...
public class Track extends TuringBaseServlet {
	...
	private void domopartner(String ip, HttpServletRequest req, String querystr, HttpServletResponse resp) throws IOException {
		...
		try {
			...
			String apikey = path.substring(path.indexOf("/click/") + 7, path.length());
			...
			if (list_union != null) {
				 ...
				 */
				// ********************************************************************
				String sub = getSub(userid, channel, aff_sub, sub1, sub2, andid, country,idfa,gaid);
				// ********************************************************************
				// ********************************************************************
				// 根据offerid 获取offer
				...
				if (adslist != null && adslist.size() > 0) {
					...
					// 替换offer参数
					String clickUrl = getClickUrl(ad, gaid, channel, idfa, sub, union);
					// 判断offer状态0or-2
					if (ad.getStatus() == 0) {
						// ********************************************************************
						// 检测cap
						...
						if (checkCap) {
							...
							if ((union.getProvider()).contains(ad.getProvider()) && !Unaffiliates.contains(union.getId() + "")) {
								send = true;
							}
							if (send) {
								// 可以直接跳转
								SendClick(resp, req, clickUrl, userid, ad, country, ip, sub, clickUrl, now);
							} else {
								PkgOffer(resp, req, userid, ad, country, ip, sub, gaid, idfa, channel, list_union);
							}
					...
					// 通过offerId没有找到offer
					resp.getOutputStream().println("No advertisement was found");
				}
			} else {
				// api地址错误
				resp.getOutputStream().println("the code is not correct, please input it again. ");
			}
	 ...
	 * @2017-10-24
	 */
	private String getClickUrl(PilotAd ad, String gaid, String channel, String idfa, String sub, SupervisorsBean union) {
		...
		// 正常点击
		if (channel != null) {
			clickUrl = clickUrl.replace("{channel}", union.getId() + "_" + channel);
		} else {
	 ...
	 * @since JDK 1.7
	 */
	private String getSub(String userid, String channel, String aff_sub, String sub1, String sub2, String andid, String country, String idfa, String gaid) {
	 ...
	 * @param list_union
	 */
	private void PkgOffer(HttpServletResponse resp, HttpServletRequest req, String userid, PilotAd ad, String country, String ip, String sub, String gaid, String idfa,
			String channel, List<SupervisorsBean> list_union) {
		try {
			...
			if (list != null) {
				...
				if (sendAd != null) {
					...
					// 正常点击
					if (channel != null) {
						clickUrl = clickUrl.replace("{channel}", userid + "_" + channel);
					} else {
					...
					try {
						...
						clickUrl = clickUrl.replace("{aff_sub}", Base64Encoder.Encoder.GetEncoded(b));
					...
					sb1.append("PkgOffer:").append("\t").append(userid).append("\t").append(ad.getId()).append("\t").append(ad.getPkg()).append("\t").append(ad.getProvider())
							.append("\t").append(country).append("\t").append(ip).append("\t").append(sendAd.getId()).append("\t").append(sendAd.getOfferid()).append("\t")
							.append(sendAd.getProvider()).append("\t").append(clickUrl).append("\t").append(os).append("\t").append(Referer).append("\t");
					TuringLog.adsLog(sb1.toString());
				} else {
					String Referer = req.getHeader("Referer");
					StringBuffer sb1 = new StringBuffer();
					sb1.append("NoCap:").append("\t").append(userid).append("\t").append(ad.getId()).append("\t").append(ad.getPkg()).append("\t").append(ad.getOfferid())
							.append("\t").append(ad.getProvider()).append("\t").append(country).append("\t").append(ad.getOs()).append("\t").append(ad.getUpdatedate())
				...
				String Referer = req.getHeader("Referer");
				StringBuffer sb1 = new StringBuffer();
				sb1.append("NoFindOffer:").append("\t").append(userid).append("\t").append(ad.getId()).append("\t").append(ad.getPkg()).append("\t").append("\t")
						.append(ad.getOfferid()).append("\t").append(ad.getProvider()).append("\t").append(country).append("\t").append(ad.getOs()).append("\t")
						.append(ad.getUpdatedate()).append("\t").append(Referer).append("\t");
				TuringLog.adsLog(sb1.toString());
	 ...
	 * @param now
	 */
	private void SendClick(HttpServletResponse resp, HttpServletRequest req, String clickUrl, String userid, PilotAd ad, String country, String ip, String sub, String clickUrl2,
		...
		StringBuffer sb1 = new StringBuffer();
		sb1.append("SendClick:").append("\t").append(userid).append("\t").append(ad.getId()).append("\t").append(ad.getOfferid()).append("\t").append(ad.getPkg()).append("\t")
				.append(ad.getProvider()).append("\t").append(country).append("\t").append(ip).append("\t").append(clickUrl).append("\t").append(os).append("\t").append(Referer)
	...
	private void incrClick(String offerid, String userid, PilotAd ad, String os) {
		...
		String clickAllKey = CacheKeyDic.UNION_AD_CLICK_REDIS_COUNT + "admin" + date;
