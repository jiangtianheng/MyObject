package com.cappuccino.offer.offers.android;
...
public abstract class BaseAndroidOffer implements Callable<Boolean> {

	protected static final Logger logger = Logger.getLogger(BaseAndroidOffer.class);
	...
	public void PutOfferTo_TemDB(List<AdTem> adlist) {
		if (adlist != null && adlist.size() > 0) {
			CacheAdTemDAO adTemDAO = SpringHelper.getBean("adTemDAO", CacheAdTemDAO.class);
	...
	public static List<Provider> updateAndroidOfferToDb(Map<String, String> blackMap) {
		...
		CacheAdTemDAO temDao = SpringHelper.getBean("adTemDAO", CacheAdTemDAO.class);
		// 获取数据库中的数据
		...
		logger.info("before black offer_tem_List size():" + offer_Tem_List.size());
		// 移除黑名单中的选项
		offer_Tem_List = removeBlackItem(offer_Tem_List, blackMap);
		logger.info("after blackOffer offer_tem  List size():" + offer_Tem_List.size());
	 ...
	 * @return
	 */
	public static List<AdTem> removeBlackItem(List<AdTem> list, Map<String, String> blackMap) {
	...
	public static Map<String, String> getBlackItem() {
		...
		try {
			blackListDao = SpringHelper.getBean("offerBlackListDAO", CacheOfferBlackListDAO.class);
	...
	public Map<String, String> getAppBlackItem() {
		...
		try {
			blackListDao = SpringHelper.getBean("blackAppDAO", CacheBlackAppDAO.class);
